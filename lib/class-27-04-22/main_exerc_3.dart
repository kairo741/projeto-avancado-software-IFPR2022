void main() {
  ...
  ///
  /// (1) auxiliar: 6 horas diárias de trabalho
  /// (2) junior: 8 horas diárias de trabalho e trabalha aos sábados recebendo 0.5x
  /// a mais do salário por hora no 6° dia de trabalho (sábado)
  ...
  }, hourWage: 5.5, workedDays: 30, role: WorkerRole.junior);

  workedHours2((int workedDays, double hourWage) => (hourWage * 6) * workedDays,
      hourWage: 5.5, workedDays: 30);

  workedHours2((int workedDays, double hourWage) {
    var wageOfADay = (hourWage * 8);
    if (workedDays >= 6) {
      var workedSats = workedDays / 6;
      var workingDays = workedDays - workedSats;
...
workedHours2(Function(int workedDays, double hourWage) calcSalary,
    {required int workedDays, required double hourWage}) {
  double fullSalary = calcSalary(workedDays, hourWage);
  print("O salário total é de R\$${fullSalary.toStringAsFixed(2)}");
}
